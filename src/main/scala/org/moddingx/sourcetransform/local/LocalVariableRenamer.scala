...
import org.moddingx.sourcetransform.transform.{TransformUtil, TransformerReader}
import org.moddingx.sourcetransform.util.inheritance.InheritanceIO
import org.moddingx.sourcetransform.util.{LanguageLevel, RenameMap, SourceUtil, Util}
...
object LocalVariableRenamer {
  ...
  def run(args: String*): Unit = {
    ...
    if (!set.has(specInput) || !set.has(specInheritance) || !set.has(specTransformer) || !set.has(specOutput)) {
      ...
      val transform = TransformUtil.createTransformer(inheritance, mappedTransformer)
      files.foreach(file => {
        val parser = createParser(file)
        parser.createAST(null) match {
          case cu: CompilationUnit =>
            val set = mutable.Set[(Int, Int, String)]()
            cu.accept(new SourceVisitor(inheritance, transform, set))
            if (set.nonEmpty) {
